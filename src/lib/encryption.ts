...
export function encryptPayload(data: any): EncryptedPayload {
  try {
    ...
    if (!encryptionKey) {
      throw new Error('NEXT_PUBLIC_ENCRYPTION_KEY is not configured in environment variables');
    }

    if (encryptionKey.length !== 64) {
      throw new Error('Invalid encryption key length. Expected 64 hex characters (256 bits)');
    }
